...
#'
#' @return
#' \item{Metric}{Negative Predictive Value parity metrics for all groups. Lower values compared to the reference group mean lower negative predictive values in the selected subgroups}
...
npv_parity <- function(data, outcome, group, probs = NULL, preds = NULL,
  ...
  p <- ggplot(val_df, aes(x=groupst, weight=val, fill=groupst)) +
    ...
    labs(x = "", y = "Negative Predictive Value Parity")
