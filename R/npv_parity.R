...
npv_parity <- function(data, outcome, group, probs = NULL, preds = NULL, outcome_levels = c("no",
    ...
    rownames(res_table) <- c("NPV", "NPV Parity")
    ...
    p <- ggplot(val_df, aes(x = groupst, weight = val, fill = groupst)) + geom_bar(alpha = 0.5) +
        coord_flip() + theme(legend.position = "none") + labs(x = "", y = "Negative Predictive Value Parity")
