context("Probabilities_vector")
library(fairness)

data("compas")
probvec <- compas$probability
...
test_that("no errors in roc_parity", {
  expect_error(roc_parity(data = compas, outcome = "Two_yr_Recidivism", group = "ethnicity",
                          probs = probvec, outcome_levels = c("no", "yes"),
                          base = "Caucasian"), NA)})
